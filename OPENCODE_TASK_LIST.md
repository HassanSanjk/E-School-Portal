# OpenCode build task list — one task per prompt

**How to use this:** give OpenCode exactly **one unchecked task at a time**,
...
- [ ] **B7. Manual test pass.** Log in as your bootstrap admin, confirm
      ...
      visit redirects to login. **This one's yours to do, not OpenCode's.**
- [ ] **B8. Security review pass #1.** Hand SCHEMA_AND_ACCESS_MATRIX.md and
      ...
      Sonnet's task, not OpenCode's — don't skip it or let OpenCode "review
      its own work."**
...
- [ ] **E5. Security review pass #2 (adversarial).** Sonnet's task, not
      OpenCode's — actively try to break access control as each role.
